...
{
	if (lines.length < placerUtils.header_height)
		return (false);

	for (let i = 0; i < placerUtils.header_height; i++)
	{
...
export function	formatNewHeader_makefile(file_path: string, header_already_exist:boolean, lines:string[], info: fs.Stats): string[]
{
	...
	let	header: string[] = new Array<string>(placerUtils.header_height + 1);
	...
	padding = placerUtils.header_length - header[3].length - ":+:      :+:    :+:    #".length;
	...
	padding = placerUtils.header_length - header[5].length - "+#+  +:+       +#+         #".length;
	...
	padding = placerUtils.header_length - header[7].length - "#+#    #+#              #".length;
	...
	padding = placerUtils.header_length - header[8].length - "###   ########.fr        #".length;
